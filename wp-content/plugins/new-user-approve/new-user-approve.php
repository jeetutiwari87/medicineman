...
class pw_new_user_approve {
	...
	public function admin_approval_email( $user_login, $user_email ) {
		...
		$content2 = ' <h3>Hello, Admin</h3>
						 <p>'.$message.'</p>
						<br/>';
		$message = email_template($content2);		
		...
		// send the mail
		wc_mail( $to, $subject, $message, $this->email_message_headers() );
	}
	...
	public function approve_user( $user_id ) {
		...
		$content2 = ' <h3>Hello, '.$user_email.'</h3>
						 <p>'.$message.'</p>
						<br/>';
		$message = email_template($content2);
		...
		// send the mail
		wc_mail( $user_email, $subject, $message, $this->email_message_headers() );
	...
	public function deny_user( $user_id ) {
		...
		// format the message
		$message = nua_default_deny_user_message($user_id);
		$message = nua_do_email_tags( $message, array(
			'context' => 'deny_user',
		) );
