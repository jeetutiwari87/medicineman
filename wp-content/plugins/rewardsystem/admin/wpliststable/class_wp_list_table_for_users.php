...
class WP_List_Table_for_Users extends WP_List_Table {
    ...
    public function total_redeem_and_expired_points($userid) {
        ...
        $currentpoints = $getusermeta[0]['availablepoints'] != '' ? round_off_type($getusermeta[0]['availablepoints']) : '0';
        ...
        $earnedpoints = $getuser[0]['availablepoints'] != '' ? round_off_type($getuser[0]['availablepoints']) : '0';
        $totalearnedpoints = $earnedpoints + (float) $deletedearnedpoints;
        $totalearnedpoint = round_off_type($totalearnedpoints);
        ...
        $expired_points = round_off_type($total_expired_points);
        $redeemmetavalue = get_user_meta($userid, 'rs_redeem_points_before_delete', true);
        $remain_redeem_points = $redeem_points[0]['redeempoints'] != '' ? round_off_type($redeem_points[0]['redeempoints']) : '0';
        $total_redeem_points = $remain_redeem_points + (float) $redeemmetavalue;
        $redeem_points = round_off_type($total_redeem_points);        
