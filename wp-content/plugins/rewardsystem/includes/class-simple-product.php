...
if (!class_exists('RSFunctionforSimpleProduct')) {
    ...
    class RSFunctionforSimpleProduct {
        ...
        public static function init() {
            ...
            add_filter('woocommerce_get_price_html', array(__CLASS__, 'display_buying_point_msg_for_product'), 999, 2);
                    ...
                    if (is_product() || is_page()) {
                       ...
                       if (is_user_logged_in()) {
                            if (get_option('rs_show_hide_buy_points_message_for_simple_in_product') == '1') {
                             ...
                             if (get_option('rs_show_hide_buy_pont_message_for_simple_in_product_guest') == '1') {
                                if (get_post_meta($post->ID, '_rewardsystem_buying_reward_points', true) == 'yes' && get_post_meta($post->ID, '_rewardsystem_assign_buying_points', true) != '') {
                                    $shop_msg = do_shortcode(get_option('rs_buy_point_message_in_product_page_for_simple'));
                                    return $price . '<br>' . $shop_msg;
                                }
                    ...
                    if (is_product() || is_page()) {
                       if (is_user_logged_in()) {
                            if (get_option('rs_show_hide_buy_points_message_for_variable_in_product') == '1') {
                             ...
                             if (get_option('rs_show_hide_buy_pont_message_for_variable_in_product_guest') == '1') {
                                if (get_post_meta($post->ID, '_rewardsystem_buying_reward_points', true) == 'yes' && get_post_meta($post->ID, '_rewardsystem_assign_buying_points', true) != '') {
                                    $shop_msg = do_shortcode(get_option('rs_buy_point_message_in_product_page_for_variable'));
                                    return $price . '<br>' . $shop_msg;
                                }
        ...
        public static function display_reward_point_msg_for_product($price, $product) {
            ...
            if ($banning_type != 'earningonly' && $banning_type != 'both') {
                ...
                if (is_object($gettheproducts) && rs_check_variable_product_type($gettheproducts) && $related_product == false) {
                    ...
                    if (get_option('rs_enable_disable_point_priceing') == '1' && get_option('rs_point_price_activated') == 'yes') {                                                   //Altered Price when Point Price
        ...
        public static function get_redeem_conversion_value() {
            ...
            $roundofftype = get_option('rs_round_off_type') == '1' ? '2' : '0';
            $price = round_off_type($updatedvalue);
        ...
        public static function rewardpoints_of_variation($variation_id, $newparentid) {
            ...
            $roundofftype = get_option('rs_round_off_type') == '1' ? '2' : '0';
            $varpoints = round_off_type($getpoints);
            return $varpoints;
        }
