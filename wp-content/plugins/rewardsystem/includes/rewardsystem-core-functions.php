...
function display_custom_message_in_point_can_earned( $points_info , $roundofftype , $message , $reward_value , $eq_price_enable , $custom_msg_enable , $custom_msg ) {
    
    if(is_array($points_info)) {
        $message .= round ( array_sum ( $points_info ) , $roundofftype ) . '&nbsp;' ;
    } else {
        $message .= round ( $points_info  , $roundofftype ) . '&nbsp;' ;
    }
...
function display_total_currency_value($getcurrentuserid) {
    ...
    return round_off_type($getoldpoints) . '(' . get_woocommerce_formatted_price(round_off_type($coupon_value_in_points)) . ')';
}
...
function calculate_point_price_for_products($product_id) {
    ...
    if (get_option('rs_enable_disable_point_priceing') == '1' && get_option('rs_point_price_activated') == 'yes') {
        ...
        if (is_object($checkproduct) && ($checkproduct->is_type('simple') || ($checkproduct->is_type('subscription')) || ($checkproduct->is_type('lottery')))) {
            if ($product_filter == '1') {
                if ($enable == 'yes') {
                    $data[$product_id] = rs_function_to_get_product_value_for_point_pricing($simple_product_type, $price_display_type, $points, $product_id);
                } else {
                    $data[$product_id] = '';
                }
                    ...
                    if ($price_display_type == '2') {
                        ...
                        if ($point_price_type == 1) {
                            if ($variablerewardpoints == '') {
                                $data[$product_id] = rs_function_to_get_category_value_for_point_pricing($newparentid);
                            } else {
                                $data[$product_id] = $variablerewardpoints;
                            }
                        } else {
...
function rs_function_to_get_product_value_for_point_pricing($simple_product_type, $price_display_type, $points, $product_id) {
    if ($price_display_type == '2') {
            ...
            if (($enablevalue == 'yes') && ($enablevalue != '')) {
                ...
                if ($display_type == '1') {
                    ...
                    if ($checktermpoints == '') {
                        ...
                        $data = get_woocommerce_term_meta($term->term_id, 'rs_category_points_price', true);
                    }
                } else {
                    $data = get_sale_and_regular_price($product_id);
                }
...
function get_sale_and_regular_price($product_id) {
    ...
    $newvalue = $product_price / $point_amount;
...
function rs_function_to_get_filtered_product($productid, $variationid, $options_to_check) {
    ...
    if ($options_to_check['applicable_for'] == '1') {
        ...
        $excludedcat = $options_to_check['excluded_categories'];
        $excluded_cat = is_array($excludedcat) ? $excludedcat : explode(',', $excludedcat);
        if (!empty($excluded_cat)) {
            foreach ($excluded_cat as $eachcat) {
                $term = get_the_terms($productid, 'product_cat');
                if (is_array($term)) {
                    foreach ($term as $termidlist) {
                        if ($eachcat != $termidlist->term_id) {
...
function check_level_of_enable_reward_point($productid, $variationid, $item, $checklevel, $referred_user, $getting_referrer, $socialreward, $rewardfor) {
 $memebershiprestrict = 'no';
    if (get_option('rs_enable_restrict_reward_points') == 'yes') {
    ...
    if (is_cart() || is_checkout()) {
        ...
        if ($mainproductdatabooking->is_type('variation')) {
            ...
            if (get_option('woocommerce_tax_display_shop') == 'incl' && get_option('woocommerce_prices_include_tax') == 'no') {
                ...
                $getregularprice = (float)$variable_product->get_price() * $itemquantity;
            }
        } else {
            $getregularprice = (float)$mainproductdatabooking->get_price() * $itemquantity;
            if (get_option('woocommerce_tax_display_shop') == 'incl' && get_option('woocommerce_prices_include_tax') == 'no') {
...
function round_off_type($points) {
