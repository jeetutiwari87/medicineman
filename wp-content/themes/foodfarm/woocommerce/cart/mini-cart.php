...
<div class="wrap-mini-cart">
            ...
            foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
                ...
                if ($_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters('woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key)) {
                    ...
                    <li class="<?php echo esc_attr(apply_filters('woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key)); ?>">
                        <?php echo apply_filters('woocommerce_cart_item_remove_link', sprintf('<a href="%s" class="remove remove-product" title="%s" data-cart_id="%s" data-product_id="' . $product_id . '"><i class="fa fa-close"></i></a>', esc_url(WC()->cart->get_remove_url($cart_item_key)), esc_html__('Remove this item', 'foodfarm'), $cart_item_key), $cart_item_key); ?>
                        <?php if (!$_product->is_visible()) : ?>
                            <?php echo str_replace(array('http:', 'https:'), '', $thumbnail) . $product_name . '&nbsp;'; ?>
                        <?php else : ?>
                            <a href="<?php echo esc_url($_product->get_permalink($cart_item)); ?>" class="cart-images">
                        ...
                        <div class="cart-info">
                            <div class="product-name">
                                ...
                                <div class="qty-cart">
									...
									//echo apply_filters('woocommerce_widget_cart_item_quantity', '<span class="quantity">' . sprintf('%s &times;', $cart_item['quantity']) . '</span>', $cart_item, $cart_item_key); 
									}else{
									echo apply_filters('woocommerce_widget_cart_item_quantity', '<span class="quantity">' . sprintf('%s &times;', $cart_item['quantity']) . '</span>', $cart_item, $cart_item_key); 
									}?>
    ...
    <?php if (!WC()->cart->is_empty()) : ?>
        ...
        <div class="cart-btn">
            <a href="<?php echo WC()->cart->get_cart_url(); ?>" class="btn button btn-default wc-forward"><?php echo esc_html__('View Cart', 'foodfarm'); ?></a>
